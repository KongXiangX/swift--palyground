...
//1.for-in
//正确： for index in 0..<3   错误： for index in 0..< 3 
//（不能在小于号 后面加 空格）
for index in 0..<3  {
...
//--------------------------------------------------------------------------------------------------------------------------//
//2. For for [initialization]; [condition]; [increment] {} 去掉了if后面的小括号，但是大括号是不能省略的
//for index = 0; index < 3; index ++  
//这种方法在 swift3.0 已经被废除
//执行顺序
...
//--------------------------------------------------------------------------------------------------------------------------//
//3. While Loops While循环，用于在循环次数不知道的情况下
...
//--------------------------------------------------------------------------------------------------------------------------//
//3. Repeat-While 等同于其他语言的do-while。
//repeat { [statements] } while [condition]  首先执行statements，然后执行condition判断，condition==true继续执行，condition==false结束运行。statements最少会被执行一次。
square = 0
diceRoll = 0
repeat {
    square += board[square]
...
//--------------------------------------------------------------------------------------------------------------------------//
//4.Conditional Statements if:简单、少数的分支 switch:复杂、分支比较多
...
//--------------------------------------------------------------------------------------------------------------------------//
//5.Switch 与其他语言不同，不用在每个case中写break，swift只会执行一个case。switch语句必须是完备的。
//这就是说，每一个可能的值都必须至少有一个 case 分支与之对应。在某些不可能涵盖所有值的情况下，你可以使用默认（default）分支满足该要求，这个默认分支必须在switch语句的最后面。case中的条件可以是多个，并且可以是任意类型
...
//--------------------------------------------------------------------------------------------------------------------------//
//6.No Implicit Fallthrough 不存在隐式的贯穿
...
//--------------------------------------------------------------------------------------------------------------------------//
//8.Control Transfer Stratements 控制转移语句 continue break fallthrough return
...
//--------------------------------------------------------------------------------------------------------------------------//
//10:Early Exit guard语法，与if类似，都是更具一个布尔语句的结果执行。
//不同的是，为了能够保证guard后的代码能够执行，你必须保障guard中的条件是true。否则将会执行else的内容（else必须存在）
...
//--------------------------------------------------------------------------------------------------------------------------//
//11.Checking API Availability 可使用值:iOS 8.3 OSX 10.10.3 watchOS
if #available(iOS 9, OSX 10.10, *) {
    print("使用ios9 API在iOS和OS X v10.10，使用API在OS X")
} else {
    print("回到早期的iOS和OS X的API")
}
