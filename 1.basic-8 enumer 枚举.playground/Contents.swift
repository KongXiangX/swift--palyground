...
//2. **Matching Enumeration Values with a Swith Statement 在Switch语句中匹配枚举值**
...
//3. **Associated Values 相关值** 可以定义 Swift 的枚举存储任何类型的相关值，如果需要的话，每个成员的数据类型可以是各不相同的。
...
//5 **Raw Values 原始值** 在Associated Values小节的条形码例子中演示了一个枚举的成员如何声明它们存储不同类型的相关值。作为相关值的替代，枚举成员可以被默认值（称为原始值）预先填充，其中这些原始值具有相同的类型。
...
//6. Initializing from a Raw Value 从原始值初始化枚举成员** 如果定义了一个有原始值类型的枚举，可以使用原始值初始化一个枚举成员，结果可能返回对应的成员，可能是nil。
